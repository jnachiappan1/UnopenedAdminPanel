...
const validationSchema = yup.object({
  content: yup.string().test("not-empty", "Content is required", (value) => {
...
const TITLES = {
  ...
  terms_and_conditions: "Edit Terms & Conditions",
  help_support: "Edit Help Support",
};

export default function DialogLegalContent(props) {
  const [editorState, setEditorState] = useState(EditorState.createEmpty());
  const [loading, setLoading] = useState(false);
  const { open, toggle, dataToEdit, onSuccess } = props;

  const type = dataToEdit?.type || "help_support";
  const dialogTitle = TITLES[type] || "Edit Legal Content";
  ...
  useEffect(() => {
    if (open) {
      setLoading(false);
      const contentValue = dataToEdit?.content?.content || "";
      console.log("Content Value:", contentValue);

      reset({ content: contentValue });
      if (dataToEdit?.content) {
        setEditorState(convertFromHTML(dataToEdit?.content?.content));
      } else {
  ...
  const onSubmit = (data) => {
    let payload = { content: data.content };
    if (!type) {
  ...
  return (
    <Dialog open={open} maxWidth="md" fullWidth>
      ...
      <DialogContent sx={{ pb: 8, px: { xs: 2, sm: 4 }, pt: { xs: 2, sm: 4 } }}>
        <form onSubmit={handleSubmit(onSubmit)} id="legal-content-form">
                  ...
                  <Editor
                    ...
                    onEditorStateChange={(state) => {
                      ...
                      try {
                        ...
                        console.error("Error converting draft to HTML:", error);
                        field.onChange("");
                      }
                    }}
                    toolbar={{
